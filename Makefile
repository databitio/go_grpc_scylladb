...
.DEFAULT_GOAL := help
.PHONY: server_grpc blog calculator help
project := server_grpc calculator blog

all: $(project) ## Generate Pbs and build

server_grpc: $@ ## Generate Pbs and build for server_grpc
...
clean: clean_server_grpc clean_calculator clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_server_grpc: ## Clean generated files for server_grpc
	${RM_F_CMD} server_grpc/${PROTO_DIR}/*.pb.go
